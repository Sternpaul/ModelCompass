# Artificial Analysis

> Source: [https://artificialanalysis.ai](https://artificialanalysis.ai)

Raw benchmark evaluations from Artificial Analysis' proprietary suite (AA API v2). Each file is one benchmark — higher is better unless noted. Values are the model's raw score on that benchmark.

Each table is a **pure ranking on a single metric**. No scores are mixed across benchmarks.

## AIME

_American Invitational Mathematics Examination accuracy (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `openai/gpt-5` | 0.957 |
| 2 | `xai/grok-4` | 0.943 |
| 3 | `openai/o4-mini` | 0.940 |
| 4 | `alibaba/qwen3-235b-a22b-instruct-2507-reasoning` | 0.940 |
| 5 | `xai/grok-3-mini-reasoning` | 0.933 |
| 6 | `openai/gpt-5-medium` | 0.917 |
| 7 | `alibaba/qwen3-30b-a3b-2507-reasoning` | 0.907 |
| 8 | `openai/o3` | 0.903 |
| 9 | `deepseek/deepseek-r1` | 0.893 |
| 10 | `google/gemini-2-5-pro` | 0.887 |

## AIME 2025

_AIME 2025 accuracy (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `openai/gpt-5-2` | 0.990 |
| 2 | `openai/gpt-5-codex` | 0.987 |
| 3 | `google/gemini-3-flash-reasoning` | 0.970 |
| 4 | `openai/gpt-5-2-medium` | 0.967 |
| 5 | `deepseek/deepseek-v3-2-speciale` | 0.967 |
| 6 | `xiaomi/mimo-v2-flash-reasoning` | 0.963 |
| 7 | `openai/gpt-5-1-codex` | 0.957 |
| 8 | `google/gemini-3-pro` | 0.957 |
| 9 | `zai/glm-4-7` | 0.950 |
| 10 | `kimi/kimi-k2-thinking` | 0.947 |

## GPQA

_Graduate-level Google-Proof Q&A diamond accuracy (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `openai/gpt-6-astra-xhigh` | 0.963 |
| 2 | `openai/gpt-6-astra` | 0.961 |
| 3 | `google/gemini-3-8-flash` | 0.953 |
| 4 | `openai/gpt-6-astra-high` | 0.949 |
| 5 | `xai/grok-4-6` | 0.949 |
| 6 | `google/gemini-3-7-flash` | 0.945 |
| 7 | `openai/gpt-5-6-sol` | 0.941 |
| 8 | `meta/muse-spark-1-3-xhigh` | 0.941 |
| 9 | `google/gemini-3-1-pro-preview` | 0.941 |
| 10 | `openai/gpt-6-astra-medium` | 0.939 |

## HLE

_Humanity's Last Exam accuracy (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `anthropic/claude-fable-5-1` | 0.591 |
| 2 | `anthropic/claude-fable-5-1-xhigh` | 0.587 |
| 3 | `anthropic/claude-fable-5-1-high` | 0.559 |
| 4 | `anthropic/claude-fable-5` | 0.555 |
| 5 | `anthropic/claude-opus-5` | 0.549 |
| 6 | `openai/gpt-6-astra` | 0.547 |
| 7 | `openai/gpt-6-astra-xhigh` | 0.546 |
| 8 | `anthropic/claude-opus-5-xhigh` | 0.544 |
| 9 | `anthropic/claude-fable-5-1-medium` | 0.538 |
| 10 | `openai/gpt-6-astra-high` | 0.531 |

## IFBench

_Instruction-following benchmark score (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `xai/grok-4-3-medium` | 0.833 |
| 2 | `xai/grok-4-20-0309` | 0.829 |
| 3 | `minimax/minimax-m3` | 0.829 |
| 4 | `nvidia/nvidia-nemotron-3-ultra-550b-a55b` | 0.814 |
| 5 | `xai/grok-4-3` | 0.813 |
| 6 | `xai/grok-4-20` | 0.812 |
| 7 | `xai/grok-4-3-low` | 0.810 |
| 8 | `alibaba/qwen3-7-max` | 0.805 |
| 9 | `nvidia/nemotron-cascade-2-30b-a3b` | 0.804 |
| 10 | `xiaomi/mimo-v2-5-pro` | 0.799 |

## LCR

_Long-context reasoning score (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `meta/muse-spark-1-2` | 0.833 |
| 2 | `kimi/kimi-k3` | 0.827 |
| 3 | `google/gemini-3-8-flash-medium` | 0.820 |
| 4 | `meta/muse-spark-1-1` | 0.813 |
| 5 | `google/gemini-3-8-flash` | 0.810 |
| 6 | `google/gemini-3-7-flash-medium` | 0.810 |
| 7 | `google/gemini-3-5-flash` | 0.810 |
| 8 | `minimax/minimax-m3` | 0.803 |
| 9 | `meta/muse-glimmer` | 0.800 |
| 10 | `google/gemini-3-7-flash` | 0.800 |

## LiveCodeBench

_LiveCodeBench coding accuracy (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `google/gemini-3-pro` | 0.917 |
| 2 | `google/gemini-3-flash-reasoning` | 0.908 |
| 3 | `deepseek/deepseek-v3-2-speciale` | 0.896 |
| 4 | `openai/gpt-5-2-medium` | 0.894 |
| 5 | `zai/glm-4-7` | 0.894 |
| 6 | `openai/gpt-5-2` | 0.889 |
| 7 | `openai/gpt-oss-120b` | 0.878 |
| 8 | `anthropic/claude-opus-4-5-thinking` | 0.871 |
| 9 | `openai/gpt-5-1` | 0.868 |
| 10 | `xiaomi/mimo-v2-flash-reasoning` | 0.868 |

## MATH-500

_MATH-500 competition-math accuracy (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `openai/gpt-5` | 0.994 |
| 2 | `openai/o3` | 0.992 |
| 3 | `xai/grok-3-mini-reasoning` | 0.992 |
| 4 | `openai/gpt-5-medium` | 0.991 |
| 5 | `anthropic/claude-4-sonnet-thinking` | 0.991 |
| 6 | `xai/grok-4` | 0.990 |
| 7 | `openai/o4-mini` | 0.989 |
| 8 | `openai/gpt-5-low` | 0.987 |
| 9 | `google/gemini-2-5-pro-05-06` | 0.986 |
| 10 | `openai/o3-mini-high` | 0.985 |

## MMLU-Pro

_MMLU-Pro knowledge accuracy (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `google/gemini-3-pro` | 0.898 |
| 2 | `google/gemini-3-pro-low` | 0.895 |
| 3 | `anthropic/claude-opus-4-5-thinking` | 0.895 |
| 4 | `google/gemini-3-flash-reasoning` | 0.890 |
| 5 | `anthropic/claude-opus-4-5` | 0.889 |
| 6 | `google/gemini-3-flash` | 0.882 |
| 7 | `anthropic/claude-4-1-opus-thinking` | 0.880 |
| 8 | `anthropic/claude-4-5-sonnet-thinking` | 0.875 |
| 9 | `minimax/minimax-m2-1` | 0.875 |
| 10 | `openai/gpt-5-2` | 0.874 |

## SciCode

_SciCode scientific coding accuracy (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `anthropic/claude-fable-5-1` | 0.620 |
| 2 | `anthropic/claude-fable-5` | 0.602 |
| 3 | `anthropic/claude-fable-5-1-xhigh` | 0.601 |
| 4 | `google/gemini-3-1-pro-preview` | 0.589 |
| 5 | `kimi/kimi-k3` | 0.587 |
| 6 | `meta/muse-spark-1-3-xhigh` | 0.586 |
| 7 | `meta/muse-spark-1-1` | 0.582 |
| 8 | `google/gemini-3-7-flash-medium` | 0.579 |
| 9 | `anthropic/claude-fable-5-1-high` | 0.576 |
| 10 | `meta/muse-spark-1-3` | 0.573 |

## TAU2

_TAU-bench agent benchmark (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `china-mobile/jt-35b-flash` | 0.991 |
| 2 | `zai/glm-5-2` | 0.991 |
| 3 | `zai/glm-4-7-flash` | 0.988 |
| 4 | `anthropic/claude-fable-5` | 0.985 |
| 5 | `stepfun/step-3-7-flash` | 0.985 |
| 6 | `zai/glm-5v-turbo` | 0.985 |
| 7 | `zai/glm-5-turbo` | 0.985 |
| 8 | `zai/glm-5` | 0.982 |
| 9 | `xai/grok-4-3` | 0.977 |
| 10 | `zai/glm-5-1` | 0.977 |

## TAU-Banking

_TAU-bench banking domain agent score (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `meta/muse-spark-1-3` | 0.524 |
| 2 | `alibaba/qwen3-8-max` | 0.513 |
| 3 | `xai/grok-4-6` | 0.507 |
| 4 | `zai/glm-5-3` | 0.503 |
| 5 | `alibaba/qwen3-8-2-4t-a95b` | 0.491 |
| 6 | `alibaba/qwen3-8-27b` | 0.480 |
| 7 | `alibaba/qwen3-8-27b-medium` | 0.474 |
| 8 | `zai/glm-5-3-flash` | 0.472 |
| 9 | `meta/muse-spark-1-3-xhigh` | 0.472 |
| 10 | `anthropic/claude-fable-5-1` | 0.472 |

## Terminal-Bench Hard

_Terminal-Bench hard sys-admin tasks (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `openai/gpt-5-6-sol` | 0.659 |
| 2 | `openai/gpt-5-6-sol-medium` | 0.629 |
| 3 | `openai/gpt-5-6-terra-xhigh` | 0.629 |
| 4 | `anthropic/claude-fable-5` | 0.629 |
| 5 | `openai/gpt-5-6-sol-high` | 0.621 |
| 6 | `openai/gpt-5-6-sol-xhigh` | 0.614 |
| 7 | `openai/gpt-5-6-sol-low` | 0.606 |
| 8 | `openai/gpt-5-5` | 0.606 |
| 9 | `openai/gpt-5-5-high` | 0.598 |
| 10 | `anthropic/claude-opus-4-8` | 0.583 |

## Terminal-Bench v2.1

_Terminal-Bench v2.1 sys-admin tasks (%)._


| # | Model | Score |
|---|-------|------:|
| 1 | `anthropic/claude-fable-5-1` | 0.914 |
| 2 | `anthropic/claude-fable-5-1-xhigh` | 0.910 |
| 3 | `openai/gpt-6-astra-high` | 0.899 |
| 4 | `anthropic/claude-fable-5-1-high` | 0.899 |
| 5 | `openai/gpt-6-astra-medium` | 0.895 |
| 6 | `openai/gpt-5-6-sol-xhigh` | 0.895 |
| 7 | `openai/gpt-6-astra-non-reasoning` | 0.891 |
| 8 | `openai/gpt-6-astra-xhigh` | 0.891 |
| 9 | `anthropic/claude-opus-5` | 0.891 |
| 10 | `openai/gpt-6-astra` | 0.884 |
