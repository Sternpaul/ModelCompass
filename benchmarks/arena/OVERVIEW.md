# LMArena (arena.ai)

> Source: [https://arena.ai](https://arena.ai)

Human-preference Elo from blind pairwise battles. Higher Elo = more preferred by human voters.

Each table is a **pure ranking on a single metric**. No scores are mixed across benchmarks.

## Agent Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `Claude Opus 5 (High)` | Net Improvement: 12.47, Confirmed Success: 15.41, Praise vs Complaint: 20.52, Steerability: 11.15, Bash Recovery: 14.23, Tool Hallucination: 1.04, Cost/Task: 19.0, Output Tokens/Task: None |
| 2 | `Claude Opus 5 (Max)` | Net Improvement: 12.0, Confirmed Success: 18.52, Praise vs Complaint: 19.13, Steerability: 6.65, Bash Recovery: 14.61, Tool Hallucination: 1.1, Cost/Task: 15.0, Output Tokens/Task: None |
| 3 | `Claude Fable 5 (High)` | Net Improvement: 11.57, Confirmed Success: 12.44, Praise vs Complaint: 21.37, Steerability: 8.99, Bash Recovery: 13.9, Tool Hallucination: 1.13, Cost/Task: 31.0, Output Tokens/Task: None |
| 4 | `Kimi K3 (Max)` | Net Improvement: 10.41, Confirmed Success: 17.97, Praise vs Complaint: 18.31, Steerability: 5.96, Bash Recovery: 8.66, Tool Hallucination: 1.14, Cost/Task: 83.0, Output Tokens/Task: None |
| 5 | `GPT 5.6 Sol (xHigh)` | Net Improvement: 9.74, Confirmed Success: 10.0, Praise vs Complaint: 22.2, Steerability: 5.77, Bash Recovery: 9.6, Tool Hallucination: 1.14, Cost/Task: 26.0, Output Tokens/Task: None |
| 6 | `Claude Opus 4.8 (High)` | Net Improvement: 9.55, Confirmed Success: 8.26, Praise vs Complaint: 21.65, Steerability: 9.01, Bash Recovery: 9.22, Tool Hallucination: 0.4, Cost/Task: 35.0, Output Tokens/Task: None |
| 7 | `GPT 5.5 (xHigh)` | Net Improvement: 8.51, Confirmed Success: 4.42, Praise vs Complaint: 13.46, Steerability: 8.86, Bash Recovery: 14.67, Tool Hallucination: 1.13, Cost/Task: 47.0, Output Tokens/Task: None |
| 8 | `Claude Opus 4.7 (High)` | Net Improvement: 8.12, Confirmed Success: 5.62, Praise vs Complaint: 12.36, Steerability: 8.44, Bash Recovery: 13.16, Tool Hallucination: 1.02, Cost/Task: 36.0, Output Tokens/Task: None |
| 9 | `GPT 5.5 (High)` | Net Improvement: 7.74, Confirmed Success: 4.09, Praise vs Complaint: 11.26, Steerability: 9.12, Bash Recovery: 13.09, Tool Hallucination: 1.14, Cost/Task: 73.0, Output Tokens/Task: None |
| 10 | `Claude Opus 4.7` | Net Improvement: 7.4, Confirmed Success: 4.99, Praise vs Complaint: 11.51, Steerability: 9.32, Bash Recovery: 10.1, Tool Hallucination: 1.08, Cost/Task: 36.0, Output Tokens/Task: None |

## Code Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `claude-opus-5-max` | 1691.000 |
| 2 | `kimi-k3-max` | 1674.000 |
| 3 | `qwen3.8-max` | 1669.000 |
| 4 | `claude-opus-5-high` | 1662.000 |
| 5 | `grok-4.6-high` | 1629.000 |
| 6 | `claude-fable-5` | 1626.000 |
| 7 | `gpt-5.6-sol-xhigh (codex-harness)` | 1619.000 |
| 8 | `glm-5.3-max` | 1597.000 |
| 9 | `gemini-3.7-flash-high` | 1588.000 |
| 10 | `deepseek-v4-pro-high-20260813` | 1582.000 |

## Document Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `claude-opus-5-high` | 1520.000 |
| 2 | `claude-opus-4-6` | 1510.000 |
| 3 | `claude-opus-4-6-high` | 1506.000 |
| 4 | `claude-fable-5` | 1504.000 |
| 5 | `claude-opus-4-7` | 1498.000 |
| 6 | `claude-opus-4-7-high` | 1497.000 |
| 7 | `gpt-5.5-high` | 1485.000 |
| 8 | `claude-sonnet-4-6` | 1483.000 |
| 9 | `gpt-5.5` | 1480.000 |
| 10 | `gpt-5.6-terra-xhigh` | 1479.000 |

## Image Edit Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `gpt-image-2 (medium)` | 1463.000 |
| 2 | `grok-imagine-image-2.0 (low)` | 1439.000 |
| 3 | `mai-image-2.6-preview` | 1420.000 |
| 4 | `muse-image` | 1406.000 |
| 5 | `mai-image-2.5` | 1401.000 |
| 6 | `seedream-5.0-pro` | 1394.000 |
| 7 | `grok-imagine-image-quality (20260519)` | 1390.000 |
| 8 | `gemini-3-pro-image-2k (nano-banana-pro)` | 1390.000 |
| 9 | `chatgpt-image-latest-high-fidelity (20251216)` | 1389.000 |
| 10 | `gemini-3-pro-image-preview (nano-banana-pro)` | 1385.000 |

## Image To Video Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `minimax-h3` | 1489.000 |
| 2 | `dreamina-seedance-2.5-720p` | 1484.000 |
| 3 | `dreamina-seedance-2.0-720p` | 1478.000 |
| 4 | `gemini-omni-flash` | 1462.000 |
| 5 | `grok-imagine-video-1.5-720p` | 1460.000 |
| 6 | `flux-3-video-20260811` | 1449.000 |
| 7 | `happyhorse-1.0` | 1442.000 |
| 8 | `wan2.7-i2v` | 1427.000 |
| 9 | `grok-imagine-video-720p` | 1415.000 |
| 10 | `veo-3.1-audio` | 1397.000 |

## Search Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `claude-opus-4-6-search` | 1253.000 |
| 2 | `gpt-5.5-search` | 1240.000 |
| 3 | `claude-fable-5` | 1237.000 |
| 4 | `claude-opus-4-7` | 1233.000 |
| 5 | `ernie-5.1` | 1226.000 |
| 6 | `claude-sonnet-4-6-search` | 1221.000 |
| 7 | `gemini-3.1-pro-grounding` | 1212.000 |
| 8 | `gemini-3-pro-grounding` | 1207.000 |
| 9 | `gpt-5.2-search` | 1206.000 |
| 10 | `grok-4.20-multi-agent-beta-0309` | 1205.000 |

## Text Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `claude-fable-5` | 1507.000 |
| 2 | `claude-opus-4-6-high` | 1505.000 |
| 3 | `claude-opus-4-7-high` | 1502.000 |
| 4 | `muse-spark-1.2 (xHigh)` | 1498.000 |
| 5 | `claude-opus-4-6` | 1497.000 |
| 6 | `claude-opus-4-7` | 1494.000 |
| 7 | `claude-opus-5-high` | 1493.000 |
| 8 | `muse-spark-1.1` | 1491.000 |
| 9 | `gemini-3.7-flash-high` | 1490.000 |
| 10 | `kimi-k3-max` | 1490.000 |

## Text To Image Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `gpt-image-2 (medium)` | 1381.000 |
| 2 | `mai-image-2.6-preview` | 1336.000 |
| 3 | `grok-imagine-image-2.0 (low)` | 1316.000 |
| 4 | `reve-2.1` | 1302.000 |
| 5 | `muse-image` | 1282.000 |
| 6 | `reve-2.0` | 1270.000 |
| 7 | `seedream-5.0-pro` | 1258.000 |
| 8 | `qwen-image-3.0-pro` | 1257.000 |
| 9 | `mai-image-2.5` | 1256.000 |
| 10 | `gemini-3.1-flash-lite-image (nano-banana-2-lite)` | 1251.000 |

## Text To Video Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `gemini-omni-flash` | 1512.000 |
| 2 | `flux-3-video` | 1494.000 |
| 3 | `dreamina-seedance-2.0-720p` | 1482.000 |
| 4 | `dreamina-seedance-2.5-720p` | 1477.000 |
| 5 | `muse-video` | 1457.000 |
| 6 | `minimax-h3` | 1453.000 |
| 7 | `happyhorse-1.0` | 1428.000 |
| 8 | `sora-2-pro` | 1364.000 |
| 9 | `veo-3.1-audio` | 1364.000 |
| 10 | `veo-3.1-audio-1080p` | 1363.000 |

## Video Edit Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `dreamina-seedance-2.5-720p` | 1411.000 |
| 2 | `minimax-h3` | 1388.000 |
| 3 | `dreamina-seedance-2.0-720p` | 1359.000 |
| 4 | `gemini-omni-flash` | 1358.000 |
| 5 | `happyhorse-1.0` | 1307.000 |
| 6 | `grok-imagine-video` | 1262.000 |
| 7 | `kling-o3-pro` | 1257.000 |
| 8 | `kling-o1-pro` | 1200.000 |
| 9 | `runway-gen4-aleph` | 1183.000 |

## Vision Arena

_Human-preference Elo._


| # | Model | Score |
|---|-------|------:|
| 1 | `claude-fable-5` | 1315.000 |
| 2 | `qwen3.8-max` | 1301.000 |
| 3 | `claude-opus-4-7-high` | 1301.000 |
| 4 | `claude-opus-4-6-high` | 1300.000 |
| 5 | `claude-opus-4-7` | 1299.000 |
| 6 | `claude-opus-5-high` | 1297.000 |
| 7 | `gemini-3.6-flash-high` | 1295.000 |
| 8 | `muse-spark` | 1294.000 |
| 9 | `claude-opus-4-6` | 1293.000 |
| 10 | `muse-spark-1.2 (xHigh)` | 1290.000 |
